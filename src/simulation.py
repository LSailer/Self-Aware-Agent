...
def check_interaction(env, threshold):
    try:
        s=env.get_state(); a=np.array(s['agent']['position'])
        for key in ['cylinder','disk','pyramid']:
            if np.linalg.norm(a-np.array(s[key]['position']))<threshold:
                return True
        return False
...
def run_simulation():
    ...
    for step in range(MAX_STEPS):
        ...
        with torch.no_grad():
            _,next_h=agent.rnn_model(agent.current_z,torch.tensor(action_array).float().unsqueeze(0).to(device),agent.current_h)
        ...
        if loss_dict:
            ...
            logger.log_metrics(step,env,action_key,action_array,
        ...
        # video annotation if needed
        try:
            frame=recorder.annotate_frame(next_raw,step,logger.rewards[-1] if logger.rewards else 0,0)
            recorder.write_frame(frame)
        except:
            pass

    print("Simulation finished.")
    recorder.close(); env.close(); logger.plot_metrics()
