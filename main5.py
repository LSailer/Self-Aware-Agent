...
sphere_id = p.createMultiBody(baseMass=1.0, baseCollisionShapeIndex=sphere_collision, baseVisualShapeIndex=sphere_visual, basePosition=[0, 0, 0.5])

# Adjust ball dynamics
p.changeDynamics(sphere_id, -1, mass=1.0, lateralFriction=0.8, restitution=0.1)

# Cube setup
cube_id = p.loadURDF("cube.urdf", [0, 0, 0.5])
p.changeDynamics(cube_id, -1, mass=1.0, lateralFriction=0.8, restitution=0.1)
...
for step in range(240):
    ...
    candidate_actions = torch.rand(5, action_dim) * 2 - 1
    ...
    best_action = candidate_actions[torch.argmax(torch.tensor(future_losses))]

    # Move the sphere
    new_sphere_pos = [
        sphere_pos[0] + best_action[0].item(),
        sphere_pos[1] + best_action[1].item(),
        max(0.5, sphere_pos[2] + best_action[2].item())
    ]
    p.resetBasePositionAndOrientation(sphere_id, new_sphere_pos, [0, 0, 0, 1])

    # Clamp velocity
    sphere_velocity, _ = p.getBaseVelocity(sphere_id)
    max_velocity = 3.0
    clamped_velocity = [max(-max_velocity, min(max_velocity, v)) for v in sphere_velocity]
    p.resetBaseVelocity(sphere_id, linearVelocity=clamped_velocity)
    ...
    predicted_next_state = world_model(torch.cat([state.squeeze(0), best_action], dim=-1).unsqueeze(0))
    ...
    predicted_loss = self_model(torch.cat([state.squeeze(0), best_action], dim=-1).unsqueeze(0))
    ...
    print(f"Step {step}, World Loss: {world_loss.item()}, Self Loss: {self_loss.item()}")
